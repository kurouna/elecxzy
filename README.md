...
* **MCP Server Integration**: Functions as a Model Context Protocol (MCP) server, allowing AI assistants (Google Antigravity, Claude Desktop/Code, etc.) to read/edit buffers and control the editor directly via the `--mcp-server` flag.
...
Add the following to your `claude_desktop_config.json` or the relevant configuration section of your AI agent (like Google Antigravity):

```json
{
  "mcpServers": {
    "elecxzy": {
      "command": "C:\\path\\to\\elecxzy.exe",
      "args": ["--mcp-server"]
    }
  }
}
...
* **MCP サーバー統合**: Model Context Protocol (MCP) サーバーとして動作可能。`--mcp-server` フラグを指定して起動することで、AI アシスタント（Google Antigravity, Claude Desktop/Code 等）から直接バッファの読み書きやエディタ操作が行えます。
...
`claude_desktop_config.json` や、Google Antigravity の設定セクションに以下の内容を追加してください：
