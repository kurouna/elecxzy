...
* **リアルタイムプレビュー**: Markdown / HTML / プレーンテキストの編集内容を反映するプレビューモード（テキストはウィンドウ幅で折り返して表示）。ソースバッファのスクロールに連動してプレビュー側もスクロールする片方向スクロール同期を備えています。Markdown プレビューは GitHub 風の表・タスクリスト・脚注を描画し、見出しにアンカーを振るので目次から飛べます。リンクは外部 URL なら既定のブラウザで、相対リンクならエディタで開きます。
