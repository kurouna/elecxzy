...
* **Cursor Visual Effects (VFX):** Add dynamic visual flair to your cursor movement. Switch between 6 unique styles (including "Light Particle", "Cyber Trace", and "Phantom Merge") or turn it off using dedicated shortcuts:
  * `C-c 1` to `C-c 6`: Switch through different VFX styles.
  * `C-c 0`: Turn VFX off.
...
`elecxzy` is now available on the Microsoft Store. You can also download binary distributions for Windows via GitHub Releases. The source repository is currently private.

👉 **[Microsoft Store](https://apps.microsoft.com/detail/9P32F2WFL69M)** ([ms-windows-store://pdp/?productid=9P32F2WFL69M](ms-windows-store://pdp/?productid=9P32F2WFL69M))
...
* **カーソルVFX:** カーソルの移動に合わせて動的な視覚効果（粒子、雷、炎、分身など）を表示します。ショートカットキーで6種類のスタイルを瞬時に切り替え可能です。
  * `C-c 1` 〜 `C-c 6`: 各種VFXスタイルを選択。
  * `C-c 0`: VFXをオフにします。
...
Microsoft Store にて公開されました。また、GitHubのリリースページからもバイナリを取得可能です。（ソースコードは現在非公開です）
