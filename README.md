...
* **Sidebar File Explorer (Filer):** A file manager (`C-x d`) integrated directly into the sidebar. Provides fast file previewing (`l` / `Right`), and directory traversal (`r`, `u`) including support for Windows system drives.
* **Outline View:** A hierarchical sidebar (`C-c o`) for Markdown and HTML files. Supports real-time preview (scrolling the editor as you move in the sidebar) and position restoration.
...
* **Settings Sidebar:** A dedicated sidebar (`C-,` or `M-x open-config`) for managing editor configurations, including font families, font sizes, and color themes.
...
* **サイドバー型ファイラ機能**: ファイルエクスプローラ (`C-x d`) を内蔵。素早くファイルの中身を確認できるプレビュー機能 (`l` / `Right`) や、Windows環境のシステムドライブにも対応したディレクトリナビゲーション (`r`, `u`) を備えています。
* **アウトラインビュー機能**: MarkdownおよびHTML用の見出し階層表示サイドバー (`C-c o`)。サイドバー内での移動に合わせたエディタのリアルタイムプレビューや、キャンセル時の位置復元に対応。
...
* **ファイル・エンコーディング管理**: 指定した時間間隔でのオートセーブや、最近開いたファイルの履歴機能 (`M-x recentf-open-files`)、非ASCII文字が含まれたファイルの保存時の自動UTF-8昇格機能、およびバッファの印刷機能 (`M-x print-buffer`) に対応。
* **サイドバー型設定画面**: エディタのフォントや配色などを変更できる設定画面 (`C-,` または `M-x open-config`)。
* **段落整形機能 (`M-q`)**: 指定した桁数 (`set-wrap-column`) でテキストをハードラップし、段落を美しく再整形する `fill-paragraph` 機能を搭載。日本語の禁則処理やサロゲートペアにも対応しています。
